...
class MeristemAnalyseWidget(QWidget):
    def __init__(self, viewer: "napari.viewer.Viewer"):
        ...
        self.btn_load_tracked.clicked.connect(self.load_tracks)'''
        ...
        self.pick_group.glayout.addWidget(self.btn_extract, 0, 0, 1, 2)

        self.qlist_cells = QListWidget()
        self.pick_group.glayout.addWidget(self.qlist_cells, 1, 0, 1, 2)
        self.btn_remove_cell = QPushButton('Remove selected cell')
        self.pick_group.glayout.addWidget(self.btn_remove_cell, 2, 0, 1, 2)
        ...
        self.btn_add_to_track = QPushButton('Initialize tracking')
        self.mtrack_group.glayout.addWidget(self.btn_add_to_track, 0, 0, 1, 2)
        ...
        self.mtrack_group.glayout.addWidget(self.track_list, 2, 0, 1, 2)
        ...
        self.mtrack_group.glayout.addWidget(self.btn_set_mother_daughter, 3, 0, 1, 2)
    ...
    def _add_connections(self):
        ...
        #self.btn_extract.clicked.connect(self.extract_cells)
        self.btn_add_to_track.clicked.connect(self.add_track_layer)
    ...
    '''def load_tracks(self):
        ...
        man_tracks = natsorted(Path(self.widget_export_directory.value).joinpath('tracked').glob('man_track*.tif'))
        man_tracks = np.stack([skimage.io.imread(t) for t in man_tracks], axis=0)
        self.viewer.add_labels(man_tracks, name='tracked_mask', blending='additive')

        self.viewer.layers['tracked_mask'].mouse_drag_callbacks.append(self.get_label_value)'''

    '''def get_label_value(self, layer, event):
        ...
        # Get value at the clicked coordinates
        value = layer.data[self.coordinates] if all(0 <= c < s for c, s in zip(self.coordinates, layer.data.shape)) else None
        if value is not None:
            self.selected_cell_ids.append(value)
            # Update the QListWidget with the selected cell ID
            self.qlist_cells.addItem(f'{value}')
            print(f"Selected cell IDs: {value}")

            track_labels = pd.read_csv(self.widget_export_directory.value.joinpath('tracked', 'man_track.txt'), 
            sep=' ', header=None, names=['id', 'birth_t','division_t', 'mother_id'])
        
            lineage_ids = [self.selected_cell_ids[-1]]
            mother_id = track_labels[track_labels.id == self.selected_cell_ids[-1]].mother_id.values[0]
            while mother_id > 0:
                new_id = track_labels[track_labels.id == mother_id].id.values[0]
                lineage_ids.append(new_id)
                mother_id = track_labels[track_labels.id == new_id].mother_id.values[0]

            self.all_lineages.append(lineage_ids)

            self.update_mask()'''

    '''def update_mask(self):
        
        selected_cells = np.zeros_like(self.viewer.layers['tracked_mask'].data)
        for lineage in self.all_lineages:
            selected_cells += np.where(np.isin(self.viewer.layers['tracked_mask'].data, lineage), self.viewer.layers['tracked_mask'].data, 0)
        if 'selected_cells' in self.viewer.layers:
            self.viewer.layers['selected_cells'].data = selected_cells
        else:
            self.viewer.add_labels(selected_cells, name='selected_cells', blending='additive')'''

    def _on_remove_cell(self):
        # This method should handle the selection of cells from the list widget
        selected_items = self.qlist_cells.selectedItems()
        if selected_items:
            selected_cell_row = int(selected_items[0].text())
            
            ind = self.selected_cell_ids.index(selected_cell_row)
            self.selected_cell_ids.pop(ind)
            self.all_lineages.pop(ind)
            self.qlist_cells.takeItem(ind)
            print(f"Removed cell ID: {selected_cell_row}")
    ...
    def _on_add_to_track(self, layer, event):
        ...
        if (self.current_track is None):
            ...
            self.current_track = pd.DataFrame(
                ...
                self.coordinates[2], label, 'None'], columns=['track_id', 't', 'y', 'x', 'label'])
        else:
            '''self.current_track = np.append(
                self.current_track,
                [[self.track_id, self.coordinates[0], self.coordinates[1], self.coordinates[2], label]],
                axis=0)'''
            self.current_track.loc[len(self.current_track)] = [self.track_id, self.coordinates[0], self.coordinates[1], self.coordinates[2], label, 'None']
        '''self.current_track = self.current_track[
            np.lexsort((self.current_track[:, 0], self.current_track[:, 1]))]'''
